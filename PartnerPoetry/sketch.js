...
// Logan S., Leif C.  (although he wasn't here the second day)
//      .-""""""-.
//    .'          '.
//   /   O      O   \
//  :           `    :
...
function generateAcrostic() { 
  ...
  for (let i = 0; i < seed.length; i++) {
    let cur = seed.charAt(i);
    for (let j = 0; j < words.length; j++) {
      if (words[j].charAt(0) === cur) {
        if (words[j] !== lastWordUsed) {
          lastWordUsed = words[j];
          poem = poem + " " + words[j];
          j = words.length;
          words.splice(j);
        }
...
function generateDiastic() {
  //Seed: Seinfeld
  // Poem: Sorry bee aviation think carefully little Special Everybody
